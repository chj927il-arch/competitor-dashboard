...
## 완료: 커뮤니티 ↔ 통계(브랜드) 연동
커뮤니티 목록에서 브랜드 배지 클릭 시 그 브랜드의 프로모션+커뮤니티 반응을 한 팝업(`#brandModal`)에 표시. `openBrandPanel(brand)`/`closeBrandPanel()` + `COMM_TO_COMP` 매핑(이투스247→etoos247, 잇올→itall_sparta, 디랩→daesung_dlab, 수만휘→sumanhui_sparta; 러셀은 매핑 없음=프로모션 없음). `#brandModal` 마크업은 `#kwModal` 옆에 위치(`kw-modal-back`/`kw-modal` 재사용), CSS는 `.brand-cols`(2열 그리드, 640px 이하 1열)·`.brand-col`·`.comm-brand.clickable`. 로컬 서버로 동작 검증 완료.
