...
## 진행 중 (WIP — 다음 세션 이어서)
- **커뮤니티 ↔ 통계(브랜드) 연동**: 커뮤니티에서 브랜드 클릭 시 그 브랜드의 프로모션+커뮤니티 반응을 한 팝업에 표시.
  - ✅ 완료(미커밋, `public/index.html`에만 있음): `openBrandPanel(brand)`/`closeBrandPanel()` 함수 + `COMM_TO_COMP`(이투스247→etoos247, 잇올→itall_sparta, 디랩→daesung_dlab, 수만휘→sumanhui_sparta; 러셀은 매핑 없음=프로모션 없음) 추가.
  - ⬜ 남은 작업: ①`#brandModal` 마크업 추가(`#kwModal` 옆, `kw-modal-back`/`kw-modal` 재사용, 안에 `#brandModalTitle`·`#brandModalBody`) ②CSS `.brand-cols`(2열, 모바일 1열)·`.brand-col`·`.brand-col-h`·`.brand-sent-row`·`.brand-comm-list`·`.brand-comm-item`·`.brand-comm-top`·`.brand-comm-tx` ③커뮤니티 목록의 `.comm-brand` 배지를 클릭가능하게(`cursor:pointer`+`onclick="openBrandPanel('첫토큰')"`, 첫 토큰=`brandTokens(r.brand)[0]`) ④커밋·푸시.
  - openBrandPanel 본문은 이미 좌(프로모션 목록)·우(반응 긍/부/평 카운트+최근 12건) 2열 HTML을 `#brandModalBody`에 렌더하도록 작성돼 있음.
