...
- responseKeywords(추천 대응): 경쟁사 움직임에 맞대응할 종합 마케팅 키워드 5~6개. 각 키워드마다 실행 예시 5개.
- proactiveKeywords(선제적): 경쟁사보다 앞서 펼칠 종합 키워드 5~6개. 각 키워드마다 실행 예시 5개.
- examples는 채널·방식이 서로 다르게 구체적으로.
- storeActions(점주 대응방안): 현장 점주가 즉시 실행할 항목 6개 이상.
...
반드시 아래 JSON 형식으로만 응답(마크다운/코드블록 없이 순수 JSON):
{
  ...
  "proactiveKeywords": [{"keyword": "키워드", "examples": ["개조식 예시 5개"]}],
...
module.exports = { runMonitoring, loadData, loadHistory, generateReport, DATA_DIR };
